...
def load_nozick_corpus(required_tag=None):
    """
    Load the Nozick/Liberty corpus into a Chroma vectorstore.
    Mirrors load_utilitarian_corpus but targets the `nozick_corpus/` directory
    and uses a distinct collection name to avoid collisions.
    """
    embedder = HuggingFaceEmbeddings(model_name="sentence-transformers/all-MiniLM-L6-v2")
    vectorstore = Chroma(collection_name="nozick_ethics", embedding_function=embedder)

    corpus_path = Path("nozick_corpus")
    count_loaded = 0
    ...
    print(f"✅ Loaded {count_loaded} Nozick corpus file(s) into Chroma.")
    return vectorstore