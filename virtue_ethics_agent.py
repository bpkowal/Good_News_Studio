...
def load_scenario_weights(scenario_id):
    ...
    return get_semantic_tag_weights(scenario_id, scenario_dir=Path("virtue_scenarios"), corpus_dir=Path("virtue_ethics_corpus"))
...
if __name__ == "__main__":
    scenario_id = "auto_virtue_0812_20250424"
    try:
        with open(f"virtue_scenarios/{scenario_id}.json") as f:
            scenario_file = json.load(f)
            query = scenario_file.get("ethical_question", "")
            if query:
                print("🧡 Virtue Ethics Response:\n", respond_to_query(query=query, scenario_id=scenario_id))
            else:
                print("⚠️ No ethical question found in the scenario file.")
    except FileNotFoundError:
        print(f"❌ Scenario file not found: virtue_scenarios/{scenario_id}.json")
