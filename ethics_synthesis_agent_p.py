...
# --- MFQ Steering Helper ---
def compute_steering_from_mfq(mfq: dict) -> dict:
    """
    Map MFQ (Care, Fairness, Loyalty, Authority, Purity) to weights over our five frameworks:
    Utilitarian, Deontological, Virtue, Care, Rawlsian. Then L1-normalize.
    Heuristic mapping (simple, explainable; tweak later if needed):
      - Care      -> Care (1.0)
      - Fairness  -> Rawlsian (0.6) + Deontological (0.4)
      - Loyalty   -> Virtue (0.75)  + Deontological (0.3)
      - Authority -> Deontological (0.7) + Rawlsian (0.3) + Virtue (0.2)
      - Purity    -> Virtue (1.1)
      - Utilitarian gets a *slightly reduced* share from Care & Fairness: 0.4*Care + 0.4*Fairness
    """
    try:
        ...
        care = fairness = loyalty = authority = purity = 0.0

    # Slightly reduce utilitarian pull from high Care/Fairness (liberal profiles),
    # and slightly increase Virtue for conservative-leaning profiles (L/A/P).
    w_util  = max(0.0, 0.4 * care + 0.4 * fairness)
    w_deon  = 0.4 * fairness + 0.3 * loyalty + 0.7 * authority
...
for name, script in AGENTS:
    ...
    try:
        ...
        #   - label text, case-insensitive, colon optional
        #   - any whitespace/newlines
        #   - capture the rest as the agent body
        pattern = r'^\s*' + re.escape(label.rstrip(':')) + r':?\s*(.*)\Z'
        m = re.search(pattern, _strip_control(raw), flags=re.IGNORECASE | re.DOTALL)

        if m:
            clean = m.group(1).strip()
        else:
            # Fallback: accept the whole stdout if non-empty, and warn to stderr
            clean = _strip_control(raw).strip()
            sys.stderr.write(
...
steering_weights = compute_steering_from_mfq(user_ethics_profile)
steering_line = (
