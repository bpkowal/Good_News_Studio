...
def load_agent_output(filepath):
    ...
    # Handle both response and answer sections and top quotes
    if "Utilitarian Answer:" in content:
        question_part, rest = content.split("Utilitarian Answer:")
    elif "Utilitarian Response:" in content:
        question_part, rest = content.split("Utilitarian Response:")
    else:
        raise ValueError("File format invalid. Must contain 'Utilitarian Answer:' or 'Utilitarian Response:'")
...
def run_critic(question: str, answer: str, scenario_meta=None, temperature=0.2, max_tokens=800):
    prompt = f"""### System:
You are a critique assistant trained to evaluate whether a utilitarian ethics response is logically sound and faithful to utilitarian principles.

Score from 0.0 to 1.0:
- 1.0 = Clear, coherent, and faithful to utilitarianism.
- 0.8 = Mostly right, with minor vagueness or omissions.
- 0.6 = Partial logic or missing core principles.
- 0.4 or less = Misaligned with utilitarian reasoning.

Checklist:
- Focuses on consequences and net utility.
- Avoids appeals to deontology or virtue ethics.
- Considers impartiality, long-term effects, and scope.
"""
...
Give a numerical score followed by a clear, specific justification in 2-4 sentences.
End with a brief summary sentence that evaluates whether the answer aligns with utilitarian reasoning.

Score:"""

    print("\n🤖 Running Critique Review...")
